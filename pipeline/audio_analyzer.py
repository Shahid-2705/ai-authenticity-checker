    ...
    def analyze(self, audio_path, progress_callback=None):
        ...
        try:
            ...
            for i, (start_t, end_t, seg_wave) in enumerate(segments):
                ...
                # Run CNN model. Normalize dB range [-80, 0] to [0, 1] first
                # (the model has no BatchNorm layers; raw dB-scale input
                # caused dead ReLUs / no learning during training) - must
                # match training/train_audio_deepfake.py's normalization.
